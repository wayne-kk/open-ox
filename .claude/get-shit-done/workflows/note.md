...
- **Global scope**: `/Users/wangwei/coding/open-ox/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/wangwei/coding/open-ox/.claude/notes/`)
...
2. Glob `/Users/wangwei/coding/open-ox/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/wangwei/coding/open-ox/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/wangwei/coding/open-ox/.claude/notes/` — works in any directory
