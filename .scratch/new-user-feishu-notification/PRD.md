...
- Use a Feishu custom group bot Webhook with signature verification enabled.
- Read the Webhook URL and signing secret exclusively from server environment variables named `FEISHU_NEW_USER_WEBHOOK_URL` and `FEISHU_NEW_USER_WEBHOOK_SECRET`.
- There is no separate enable switch. Delivery is enabled only when both values are present; missing configuration produces a server-side configuration warning and otherwise behaves as a no-op.
- Generate the Feishu request timestamp and signature server-side for each request.
