...
import type { ModifyRecord } from "@/app/[locale]/studio/hooks/useBuildStudio";
import { DiffPatchLines } from "@/app/[locale]/studio/components/DiffPatchView";
import { formatModifyDetailsSummary } from "@/app/[locale]/studio/lib/modifyHistoryView";
    ...
    <div className="flex h-full min-h-0 flex-col bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(0,0,0,0.22))]">
      <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-2.5">
        <button
          ...
          className="flex items-center gap-1.5 rounded-md border border-border bg-muted/40 px-2.5 py-1.5 font-mono text-[10px] text-muted-foreground/80 transition-colors hover:border-border hover:text-foreground"
        >
        ...
        <div className="relative flex items-center gap-1">
          ...
          <button
            ...
            className={cn(
              ...
              changesOpen
                ...
                : "border-border bg-muted/40 text-muted-foreground/80 hover:border-border hover:text-foreground"
            )}
            title="本步改动的文件"
          >
          ...
          {changesOpen ? (
            <div className="absolute right-0 top-[calc(100%+6px)] z-20 w-[min(360px,70vw)] overflow-hidden rounded-xl border border-border bg-popover text-popover-foreground shadow-[var(--box-shadow-neon-lg)]">
              <div className="border-b border-border px-3 py-2 font-mono text-[9px] uppercase tracking-widest text-muted-foreground/60">
                本步文件
              </div>
              <div className="max-h-[240px] overflow-y-auto scrollbar-unified">
                {diffs.map((d) => (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-2 px-3 py-2 text-left font-mono text-[11px] transition-colors hover:bg-muted",
