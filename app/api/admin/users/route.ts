...
import { getDisplayName, type UserActivityStatus } from "@/lib/admin/userDirectory";
import { createSupabaseServiceRoleClient } from "@/lib/supabase/service-role";

function parseActivityStatus(raw: string | null): UserActivityStatus | "all" {
  if (raw === "active" || raw === "silent" || raw === "churned" || raw === "never") return raw;
  return "all";
}
...
export async function GET(req: NextRequest) {
  ...
  const perPageRaw = Number.parseInt(searchParams.get("perPage") ?? "20", 10) || 20;
  ...
  const activation =
    activationRaw === "activated" || activationRaw === "not_activated" ? activationRaw : "all";
  const status = parseActivityStatus(searchParams.get("status"));

  try {
    const directory = await fetchAdminUserDirectory({
      q,
      page,
      perPage,
      role,
...
export async function POST(req: NextRequest) {
  ...
  if (!userId && !name) {
    return NextResponse.json({ error: "userId or name required", success: false }, { status: 400 });
  }
  ...
  if (!match && name) {
    ...
    if (matches.length === 0) {
      return NextResponse.json({ error: "User not found by name", success: false }, { status: 404 });
    }
    if (matches.length > 1) {
      return NextResponse.json(
        {
  ...
  if (!match) {
    return NextResponse.json({ error: "User not found by userId", success: false }, { status: 404 });
  }
  ...
  if (upsertError) {
    return NextResponse.json({ error: upsertError.message, success: false }, { status: 500 });
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (!userId) {
    return NextResponse.json({ error: "userId required", success: false }, { status: 400 });
  }
