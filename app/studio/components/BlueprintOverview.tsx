    ...
    return (
        <div className="space-y-4 text-[12px]">
            ...
            <div className="space-y-1">
                ...
                <div className="text-[13px] font-medium text-foreground">{blueprint.brief.projectTitle}</div>
                <div className="leading-5 text-muted-foreground">{blueprint.brief.projectDescription}</div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl bg-white/4 px-3 py-2">
                        ...
                        <ul className="space-y-1">
                            {(blueprint.brief.productScope as any).inScope?.map((item: string, i: number) => (
                    ...
                    <div className="rounded-xl bg-white/4 px-3 py-2">
                        ...
                        <ul className="space-y-1">
                            {(blueprint.brief.productScope as any).outOfScope?.map((item: string, i: number) => (
                                <li key={i} className="flex gap-1.5 leading-4 text-muted-foreground">
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-2">
                    {blueprint.brief.capabilities.map((cap) => (
            ...
            <div className="space-y-2">
                ...
                <div className="rounded-xl bg-white/4 px-3 py-2 space-y-2">
                    <div>
                        ...
                        <span className="text-muted-foreground">{blueprint.experience.designIntent.style}</span>
                    </div>
                    <div>
                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground/60">Colors · </span>
                        <span className="text-muted-foreground">{blueprint.experience.designIntent.colorDirection}</span>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                        {blueprint.experience.designIntent.mood.map((m) => (
                            <span key={m} className="rounded-full bg-white/8 px-2 py-0.5 text-[11px] text-foreground/70">{m}</span>
                        ))}
                        {blueprint.experience.designIntent.keywords.map((k) => (
            ...
            <div className="space-y-2">
                <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary/80">
                    Site · {blueprint.site.pages.length} pages
                </div>
                <div className="space-y-2">
