...
import type { StepTrace } from "../../studio/types/build-studio";
...
export function TracePanel({ trace }: { trace: StepTrace }) {
    const tabs = ([
        { id: "input" as const, label: "Input", available: trace.input != null },
        { id: "output" as const, label: "Output", available: trace.output != null },
        { id: "llm" as const, label: "LLM Call", available: trace.llmCall != null },
        { id: "validation" as const, label: "Validation", available: trace.validationResult != null },
    ] satisfies Array<{ id: TraceTab; label: string; available: boolean }>).filter((t) => t.available);
