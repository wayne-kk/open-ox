  ...
  return (
    <div className={`rounded-xl hover:bg-white/3 ${isToolCall ? "border border-blue-400/10 bg-blue-400/3" : ""}`}>
      ...
      >
        ...
        <span className={`shrink-0 pt-0.5 text-[10px] transition-transform ${open ? "rotate-180" : ""} text-muted-foreground/60`}>
          ▾
        </span>
      </div>
        ...
        <div className="mx-2 mb-2 rounded-lg border border-white/6 bg-white/[0.03] px-3 py-2.5 text-[11px] leading-5">
          {isToolCall && toolDetail ? (
            <div className="space-y-2">
              ...
              <div>
                <div className="text-[10px] text-muted-foreground/70 mb-1">Args</div>
            ...
            <>
              ...
              <div className={`mt-1.5 ${step.status === "ok" ? "text-foreground/85" : "text-red-300/90"}`}>
                {narrative.output}
              </div>
              {narrative.note && (
                <div className="mt-1.5 break-words text-muted-foreground/70 italic">
                  {narrative.note}
                </div>
              )}
