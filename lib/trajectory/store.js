...
export async function createTrajectoryRun(input) {
...
export async function appendTrajectoryEvent(runId, eventInput) {
    ...
    const nextStatus = event.event_type === "run_end" ? "finished" : "running";
...
export async function listTrajectoryRunEvents(runId) {
...
export async function createRunEndEvent(runId, taskId, actor, payload, phase = "finalize") {
...
export async function listTrajectoryRuns(limit = 20, offset = 0) {
...
export async function getTrajectoryRun(runId) {
    const result = await supabase
        .from("trajectory_runs")
        .select("run_id,task_id,schema_version,status,last_seq,created_at,updated_at,ended_at")
        .eq("run_id", runId)
...
export async function getLatestEvaluatorRun(runId) {
    const result = await supabase
        .from("evaluator_runs")
        .select("id,run_id,verdict,score,failure_type,summary,created_at")
        .eq("run_id", runId)
