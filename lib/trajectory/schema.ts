export type TrajectoryPhase = "setup" | "planning" | "execution" | "verification" | "finalize";
...
const ALLOWED_PHASES: readonly TrajectoryPhase[] = [
  "setup",
  "planning",
  "execution",
  "verification",
...
const ALLOWED_EVENT_TYPES: readonly TrajectoryEventType[] = [
  "run_start",
  "run_end",
  "thought",
  "message",
...
export function validateRunStartInput(value: unknown): RunStartInput {
  if (!isRecord(value)) {
    throw new Error("Body must be a JSON object");
  }
  const { task_id, goal, task_spec_ref, environment, success_criteria, meta } = value;

  if (!isNonEmptyString(task_id)) throw new Error("Missing or invalid 'task_id'");
  if (!isNonEmptyString(goal)) throw new Error("Missing or invalid 'goal'");
  if (task_spec_ref !== undefined && !isNonEmptyString(task_spec_ref)) {
    throw new Error("Invalid 'task_spec_ref'");
  }
  if (!isRecord(environment)) throw new Error("Missing or invalid 'environment'");
...
export function validateTrajectoryEvent(value: unknown): TrajectoryEvent {
  if (!isRecord(value)) {
  ...
  if (!isNonEmptyString(schema_version)) throw new Error("Missing or invalid 'schema_version'");
  if (!isNonEmptyString(task_id)) throw new Error("Missing or invalid 'task_id'");
  if (!isNonEmptyString(run_id)) throw new Error("Missing or invalid 'run_id'");
  if (!isNonEmptyString(event_id)) throw new Error("Missing or invalid 'event_id'");
  if (typeof seq !== "number" || !Number.isInteger(seq) || seq <= 0) {
    throw new Error("Missing or invalid 'seq' (must be positive integer)");
  }
  if (!isIsoDateString(ts)) throw new Error("Missing or invalid 'ts'");
  if (!isNonEmptyString(phase) || !ALLOWED_PHASES.includes(phase as TrajectoryPhase)) {
    throw new Error("Missing or invalid 'phase'");
  }
  if (!isNonEmptyString(event_type) || !ALLOWED_EVENT_TYPES.includes(event_type as TrajectoryEventType)) {
    throw new Error("Missing or invalid 'event_type'");
  }
  if (!isNonEmptyString(actor) || !ALLOWED_ACTORS.includes(actor as TrajectoryActor)) {
  ...
  return {
    ...
    seq,
    ts,
    phase: phase as TrajectoryPhase,
    event_type: event_type as TrajectoryEventType,
    actor: actor as TrajectoryActor,
    payload,
