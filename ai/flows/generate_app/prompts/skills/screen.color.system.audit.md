# Skill｜Theme Color Diagnosis & Optimization
...
2. Check against 7 diagnostic rules one by one to identify specific issues
...
**Detection method**: Count all UI scenarios where each non-gray color appears (CTA fill, text highlight, card border, icon tint, tag background, price text, etc.). Any instance decorated with a glow / halo / neon-shadow effect counts as an additional role signal on top of its semantic role. If the same color appears in ≥3 different scenarios, it is overloaded.
...
**Detection method**: Compare each color's combined visual weight = `area × saturation × contrast-with-background`. If Destructive/Warning color weight > Primary color weight, it is inverted.
...
**Definition**: The brand Primary color and a semantic color (Success/Destructive) have hues too close together (hue difference < 30°), making it impossible for users to distinguish brand actions from status feedback.
...
### Rule 7: Saturation & Glow Overload 🔴

**Definition**: One or more colors appear at extreme saturation (HSL S > 85%) and/or carry glow / halo / neon-shadow effects across multiple locations, producing a visually "screaming" page.

**Detection method**:

- Flag any non-gray color whose HSL saturation exceeds 85% AND renders across large areas or multiple elements.
- Count glow / halo / neon-shadow occurrences on the page. More than 1 element carrying such an effect (regardless of color) is overload.
- On dark themes, also flag pure neon hues like `#FF00FF`, `#00FFFF`, `#FF0000`, or similar max-chroma values used at full opacity.

**Why it matters**: High saturation plus glow is a visual-weight multiplier. When stacked across a page (AI-generated mobile UI does this constantly), every accent element competes for attention, the Primary CTA loses its uniqueness, and the page reads as gaudy rather than branded.

**Typical symptoms**:

- Neon magenta used at full saturation for CTA, selected nav, tab pill, and link text — each with its own glow halo
- Dark-theme page where the Primary renders at S > 90% and is repeated as a background fill
- Multiple elements (FAB + nav icon + badge) all carrying the same glow effect

**Fix direction**:

- Drop Primary saturation to 60–75% on dark themes (65–80% on light themes).
- Keep glow / halo on **exactly one** element per screen — normally the Primary CTA — and strip it from every other usage.
- Replace repeated full-color instances with muted variants (10–20% opacity fills, lower-saturation text tints).
...
Check rules 1→7 in order. For each finding, output:
...
5. **Dark theme special rules**:
    - Primary saturation must stay ≤ 75%; avoid large-area high-saturation backgrounds.
    - At most 1 element on the entire screen may carry a glow / halo / neon-shadow effect — reserve it for the Primary CTA.
    - Badges, dots, tab indicators, and link text must use muted variants (opacity or lowered saturation), never the raw Primary hue.
    - Use light gray text instead of pure white; use low-opacity white for borders instead of solid colors.
    - Neon exclusion list — these hues must be desaturated before use on dark backgrounds: `#FF00FF` / `#FF1493` (hot magenta), `#00FFFF` (pure cyan), `#FF0000` (pure red), `#00FF00` (pure green), and similar max-chroma values.
...
These 5 principles form the theoretical foundation for all diagnosis and fixes:
...
The Primary color's combined visual weight (`area × saturation`) must be the largest among all non-gray colors. Semantic colors (red/green/yellow) appear only locally under trigger conditions — never as persistent large-area renders.
...
### Principle 5: Glow Scarcity

Glow, halo, and neon-shadow effects are visual-weight multipliers. The entire page may bind such an effect to **at most one** element — normally the Primary CTA. Once repeated across multiple elements, every occurrence loses its emphasis and the page reads as neon clutter rather than focused branding.
...
- [ ]  At most 1 element on the page carries a glow / halo / neon-shadow effect
- [ ]  Primary saturation falls within theme-appropriate range (dark ≤ 75%, light ≤ 80%)
