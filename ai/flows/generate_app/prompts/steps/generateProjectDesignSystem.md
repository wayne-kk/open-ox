...
This document empowers bespoke section design while keeping the app experience visually coherent.
...
- `font-display` → 首屏主标题 only (max 1 per screen)
...
Every Section must use this exact two-layer structure (mobile-first baseline):
...
- Default: `container mx-auto px-4 py-14 sm:px-6 lg:px-8`
- First-screen / strong visual sections: `py-16` (mobile) and may scale moderately on larger breakpoints
...
- **Grid**: [mobile columns / tablet columns / desktop columns]
- **Base spacing unit**: [e.g., 4px — describe how multiples are used]
- **Section vertical rhythm**: `py-14` standard / `py-16` first-screen sections as baseline
- **Density**: [`compact` definition] / [`comfortable` definition] — specify which sections use which
- **First-Screen Rule**: First-screen section should include a clear visual anchor (product preview / illustration / graphic system), but must preserve mobile readability and touch ergonomics.
