...
- **除用户明确提供的图片 URL 外，禁止使用 Unsplash、Pexels、Pixabay、Picsum 或任何其他第三方图库 / 随机占位图 URL。** 所有新配图必须优先通过 `generate_image` 生成，并使用工具返回的 `/images/...` 项目相对路径。
- **相对路径只能来自工具返回值，不得自行编造占位路径。** 正确流程是：先调用 `generate_image`，立即把工具返回的 `/images/...` 路径写入组件；工具会在后台将生成结果保存到该路径对应的 `public/images/` 文件。
