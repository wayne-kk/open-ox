...
- 当 `chromeForm=sidebar` 时，桌面端侧栏必须保持在视口内：侧栏根元素使用 `sticky top-0 h-dvh self-start`（或语义等价实现），禁止作为普通文档流长列随主页面滚出视口。
- 侧栏内容超过视口高度时，由侧栏自身使用 `overflow-y-auto` 滚动；品牌区、导航区和底部账户区可按需拆分，但不得通过增高整个页面来容纳侧栏内容。
- App Shell 使用横向 flex/grid；主内容使用 `min-w-0 flex-1`（或等价 grid track）独立增长。**主内容不得被侧栏高度撑长**，侧栏也不得覆盖或挤压主内容。
- `sticky` 侧栏的祖先不得设置会改变其包含块或使其失效的 `overflow: hidden/auto/scroll`、`transform`、`filter`；页面滚动应由文档/主内容承担。
- 移动端不得保留整高侧栏占据正文宽度；应切换为可关闭抽屉、紧凑顶栏或设计系统规定的移动导航，并保持焦点与关闭操作可用。
