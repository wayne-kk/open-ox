...
export function buildIntentAgentControlTools(): ChatCompletionTool[] {
  ...
  const yieldTool: ChatCompletionTool = {
    type: "function",
    function: {
      name: "yield_to_user",
      description:
      ...
      parameters: {
        ...
        properties: {
          ...
          message: {
            type: "string",
            description: "面向用户的主文案（可含换行）。短；不要在正文复述快捷按钮。",
          },
          suggested_replies: {
            type: "array",
            items: { type: "string" },
  ...
  if (directionLock) return [yieldTool];

  const commitTool: ChatCompletionTool = {
    type: "function",
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          site_outline: {
            type: "object",
            description: "可选：用户确认的 SiteOutline。",
          },
        },
