...
- 禁止使用 clip-path、polygon()、有机形状蒙版、花瓣形/椭圆形裁剪。所有形状只能用 border-radius。
- 图片展示一律使用矩形或圆角矩形，不要用圆形、椭圆、异形裁剪。
- 背景以纯色为主。禁止添加噪点、grain、纹理叠加，除非设计系统明确定义了。
- 相邻 Section 的背景必须有明显色调区分，避免视觉上连成一片。
