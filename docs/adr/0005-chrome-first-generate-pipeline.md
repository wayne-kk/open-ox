...
1. **Default generation order is chrome-first**: Plan agent selects `chromeForm` from the brief (no productType→form lookup tables in code or prompts) → `architect_scaffold_agent` writes a real shell (`app/layout.tsx` + `components/chrome/**`) when a global form is chosen (or when form is unspecified — Scaffold decides) → Page Agents implement content only (may run in parallel) → `chrome_optimize_agent` is **link polish only**.
2. **`chrome-deferred` (pass-through layout until after all pages) is no longer the default path.** It remains only as an implementation detail inside screenshot-replicate / agent-chosen `page-local` / `none` flows.
3. **Exceptions are agent plan results**, not post-hoc detectors or product-type heuristics:
   - Screenshot replicate → page owns chrome (existing).
   - Agent chose `chromeForm ∈ {page-local, none}` → skip global chrome mount.
