...
User 消息侧注入设计系统、预读的 `layout.tsx` / `globals.css` / 目录树、页级 `pageDesignPlan`，以及（若存在）**Hero skill** 全文。每个 worker 通过 [`PageBuildSession`](product/page-build-session-v2-architecture.md) 自主决定页面组件边界，并用统一的 `create_page_file` 增量写入目标路由或限定命名空间内的页面组件；运行时不声明组件图或锁定文件顺序，只负责所有权、revision-safe 编辑、校验、生图与最终完成条件。Page 会话强制使用 managed AgentContext；完成由 `FileSession.stopDecision()` 根据目标产物、诊断和图片契约确定。全局 `next build` / 修复仍由流水线统一调度。
