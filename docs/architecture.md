...
Step 11: repair_build × 0-5           ← LLM（构建失败时自动修复，最多 5 轮）
```
...
布局形态（顶 nav / sidebar / 工具栏 / 页脚 / 无 chrome 等）由下游页面实现 Agent 根据产品形态自行决定。`plan_project` 与 `analyze_project_requirement` 都不预先指定全局 chrome。
...
```typescript
// 最多 5 轮修复（maxRepairAttempts = 5；最后一轮仅 build，不再触发 repair）
