...
| 用户可见 URL 形态 | **Storage 直连**：`p/{projectId}/index.html` 的 `getPublicUrl`（与 key 一致）。**经本站代理**（`app/site-previews/...`）：可对外展示为 `…/site-previews/{projectId}/`（尾部 `/`，由路由映射到同一路径下的 `index.html` 对象，地址栏不出现 `.html`）。 |
...
**直连 Storage 时**：入口即 **`…/index.html` 的 public URL**（或自定义域反代到该对象）；无需依赖「仅尾部斜杠」的目录索引。

**经 Open-OX 预览代理时**：产品可向用户返回 `https://{站点}/site-previews/{projectId}/`（无 `index.html` 后缀）；实现上仍从 Storage 读取 `p/{projectId}/index.html`。
