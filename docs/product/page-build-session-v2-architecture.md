...
workspace failure budget. Once a path is present, a repeated component create never reaches the
workspace mutation layer: the runtime adopts and reads the existing artifact, then narrows the tool
surface to a revision-safe local edit.
