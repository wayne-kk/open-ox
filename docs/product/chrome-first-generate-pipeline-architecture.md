...
1. **默认 chrome-first**：Plan Agent 根据 brief **自行选定** `chromeForm`（禁止 productType 查表）→ Chrome Scaffold **一定**先落真实壳到 `components/chrome/**`（Nav / Sidebar / Footer / tabs；`unspecified` 时由 Scaffold 自决）→ Page Agent **只填内容**（可并行），**禁止写壳**。
2. **`chrome_optimize_agent` 降为 link polish**：修正路由 / section 锚点，不换壳、不发明第二套 Nav。
3. **例外仅截图复刻**：screenshot replicate → pass-through layout，由页面复刻参考图中的壳。**已删除 `page-local`**；`none` 仍由 Chrome 拥有极简壳。
4. **共享契约串行预写**：list/detail 的实体字段与共享组件路径先落盘，再并行写页。
5. **努力档**拨模型/thinking，不靠「整页互不知晓地并行」换速度。
6. **非所有权并行**：生图、`install_deps` 等可与页生成重叠。
7. **禁止场景硬编码**：代码与 prompt 不得把 dashboard/feed/官网等映射到固定壳；不得用页内 regex 检测强制 skip/mount。
...
`top-nav` | `top-nav+footer` | `sidebar` | `bottom-tabs` | `none`

- **一律**先跑 `architect_scaffold_agent`（壳拆到 `components/chrome/**` 再挂 layout）。
- `none` → Scaffold 仍拥有 layout，落极简/无营销导航壳（**不是** Page 写壳）。
- `unspecified` 或 Plan 未给出 → 仍跑 Scaffold，由 Scaffold Agent **自行决定**壳形态（代码**不得**按 productType 推断）。
- screenshot replicate → 唯一 pass-through 例外。
- **`page-local` 已删除**（遗留字符串 normalize 为 `unspecified`）。

缺省时**不要**用 productType 启发式强行填 `chromeForm`；那是 Agent 的规划结果，不是场景查表。
...
- [ ] 单页沉浸式：仍有 Chrome 壳（Scaffold 落盘），Page 不写 Nav/Footer  
