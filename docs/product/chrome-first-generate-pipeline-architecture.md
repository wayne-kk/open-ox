...
1. **默认 chrome-first**：Plan Agent 根据 brief **自行选定** `chromeForm`（禁止 productType 查表）→ Chrome Scaffold **先**落真实壳（`unspecified` 时由 Scaffold 自决）→ Page Agent **只填内容**（可并行）。
2. **`chrome_optimize_agent` 降为 link polish**：修正路由 / section 锚点，不换壳、不发明第二套 Nav。
3. **例外是计划结果**：screenshot replicate、Agent 显式选择 `chromeForm ∈ {page-local, none}` → 不挂全局壳。
